...
5. Group discovered thread IDs into idempotent Temporal activities of at most 10 threads. Each bounded worker fetches a thread with `threads.get(format=full)`, decodes Gmail's already-parsed part tree, fetches any externally referenced attachment bytes, and atomically commits that snapshot: every returned message is upserted, previously stored messages on the same thread that Gmail no longer returned are deleted with object-cleanup manifests, and the PostgreSQL checkpoint completes in the same transaction. A retry skips completed threads and safely re-upserts an unfinished thread.
...
Inline forwards submit `forwardOfMessageId` and the authored body to the compose-draft API. The API resolves the original message within the authenticated user and selected connected account before admitting a draft write, then appends its complete quoted content. The shared 10,000-character body limit applies only to authored text; a forward may contain no authored text. Original content never travels back in the compose request, is not truncated, and uses the same plain-text/HTML fallback formatting as the collapsed preview. Bcc headers and attachments from the original message are not forwarded. Reply and forward source identifiers are mutually exclusive. Forward source identity participates in the draft write fingerprint, and retries retain the existing provider draft and send idempotency keys.

## Thread-label analysis

Initial and repair imports label each thread through exactly one path, enforced by the pending-to-running analysis state machine and the per-account label advisory lock. A thread is eligible only when its atomic full-thread checkpoint is complete, at least one stored message currently carries Gmail `INBOX`, no selected message carries `SPAM` or `TRASH`, its analysis is pending, and no manual Invook assignment exists. When live classification is configured, eligible threads whose latest message falls within `MAIL_LABEL_HOT_WINDOW_DAYS` of the run's creation (zero disables) are reserved for the tenant's `live` label lane as each storage batch completes, newest-first, at most `MAIL_LABEL_HOT_WINDOW_MAX_THREADS` per run; the reservation happens under the account label lock before Batch admission, and a failed or unconfigured live analysis returns the thread to the pending Batch pool. During synchronization, account-locked admission starts at 100 eligible pending threads and claims at most 2,000, newest-first by latest message, into a durable `thread_label_batch_submissions` manifest before any provider call. Input construction is bounded by OpenAI's 200 MB file limit and the configured per-Batch input-token ceiling. Only one queued, preparing, or submitted initial label Batch exists per account; its terminal result admits the next durable slice, and Gmail finalization flushes the last group below 100. Gmail storage remains on the tenant's `bulk` lane while Batch submission and result work runs durably and mailbox events make completed labels appear incrementally.
