...
Initial Memory waits for the watch-first sequence to complete. Recent thread-label work starts while Gmail discovery is still running, as soon as every currently discovered message for a candidate thread has finished storing:
...
10. Flush the remaining partial historical thread-label Batch, then publish initial Memory derivation work.
